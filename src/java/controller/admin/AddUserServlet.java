...
public class AddUserServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String duplicateError = dao.checkDuplicateForAdmin(username, email, phone);
            if (duplicateError != null) {
                request.setAttribute("errorMsg", duplicateError);
            ...
            if (dao.addUserByAdmin(username, hashedPassword, email, fullName, phone, address, gender, birthday, roleId)) {
                request.getSession().setAttribute("successMsg", "Đã tạo tài khoản mới thành công!");
                response.sendRedirect(request.getContextPath() + "/admin/accounts");
            } else {
                request.setAttribute("errorMsg", "Thêm thất bại do lỗi cơ sở dữ liệu!");
                request.getRequestDispatcher("/admin/add-user.jsp").forward(request, response);
            }
            
        } catch (Exception e) {
            request.setAttribute("errorMsg", "Có lỗi xảy ra trong quá trình xử lý!");
            request.getRequestDispatcher("/admin/add-user.jsp").forward(request, response);
        }
    }
